...
describe('Loan', () => {
    ...
    it('should save a loan request', async () => {
        ...
        expect(treasuryState.totalBorrowersStake).toBeGramValue('151')
    ...
    it('should participate in election', async () => {
        ...
        expect(treasuryState.totalCoins).toBeGramValue(deadShares + toNano('700000'))
    ...
    it('should handle external participate message only once', async () => {
        ...
        try {
            ...
            expect((e as EmulationError).exitCode).toEqual(err.unableToParticipate)
        }
        ...
        expect(treasuryState.totalStaking).toBeGramValue('0')
        expect(treasuryState.totalUnstaking).toBeGramValue('0')
        expect(treasuryState.totalBorrowersStake).toBeGramValue('483')
        expect(treasuryState.participations.size).toEqual(1)
    ...
    it('should handle external finish message only once', async () => {
        ...
        try {
            await treasury.sendFinishParticipation({ roundSince: until1 })
            throw new Error('failed')
        } catch (e) {
            expect((e as EmulationError).exitCode).toEqual(err.notReadyToFinishParticipation)
        }
    ...
    it('should correctly set sort keys', async () => {
        ...
        expect(keys[2]).toEqual((1n << 120n) - 1n)
    })
})
